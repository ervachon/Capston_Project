...
First you choice on the sidebarPanel which corpus you want to use: with or without stop words. **WAIT** a few seconds until **"LOAD CORPUS"** is gone (top right on the screen). Next write a sentence on the "sentence" box and wait until the eight buttons labels are update with the best predict words. Then you can click on a button you add the word to your sentence. 

### <u>Memory optimisation</u>
To optimize the memory of [The Shiny App](https://ervachon.shinyapps.io/Capston_Project), we truncate the n-gram corpus, for example  we select only 2-gram with minimum 4 occurences.
